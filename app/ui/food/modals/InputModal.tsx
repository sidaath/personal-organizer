...
export default function InputModal({
  ...
  fields?: (ModalNumberInput | ModalTextInput)[];
    ...
    <div className="size-full z-40 fixed absolute top-0 left-0 right-0 bottom-0 overflow-x-hidden transition-all overflow-y-auto pointer-events-none">
      <div className="sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div className="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto dark:bg-neutral-800 dark:border-neutral-700 dark:shadow-neutral-700/70">
          ...
          <div className={`p-4 overflow-y-auto grid grid-cols-${cols} gap-2`}>
            {fields?.map((object) => {
              if (object.inputType === "text") {
                return (
                  <TextInput
                    ...
                    placeholder={object.placeholder || "INPUT"}
                    inputValue={object.value}
                    setInputValue={object.setValue}
                    type="text"
                  />
                );
              }
              if (object.inputType === "number") {
                return (
                  <NumberInput
                    id={object.id}
                    name={object.name}
                    disabled={object.disabled}
                    number={object.value}
                    setNumber={object.setValue}
                    placeholder={object.placeHolder || "NUMBER"}
                    required={object.required}
                    key={object.id}
                  />
                );
